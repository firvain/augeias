...
def get_sensor_data():
    ...
    if not m2 is not None and isinstance(m2, DataFrame) and not m2.empty:
        m2 = rename_pandas_columns(m1, {
    ...
    if not m3 is not None and isinstance(m3, DataFrame) and not m3.empty:
    ...
    m4 = Sensors_Mongo.get_mongo_data('323c14d3d8ad3e919ce9699403cbc0ca2ead8c5b',
                                      ['Conductivity', 'RDO0', 'TSS', 'Turbidity'],
                                      past_hours=PAST_HOURS)

    if m4 is not None and isinstance(m4, DataFrame) and not m4.empty:
        m4 = rename_pandas_columns(m4, {'Conductivity0-μS/cm': 'Conductivity', 'RDO0-mg/l': 'RDO',
                                        'TSS0-mg/l': 'TSS', 'Turbidity0-NTU': "Turbidity"})
    ...
    m5 = Sensors_Mongo.get_mongo_data('59a85c7da55bf1bf6e784675c060a2e71ee2373a',
                                      ['channel', 'sign', 'value'],
                                      past_hours=PAST_HOURS)
    if not m5 is not None and isinstance(m5, DataFrame) and not m5.empty:
